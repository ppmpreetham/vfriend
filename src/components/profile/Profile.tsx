...
import { resetAllStores, viewAllStores, getUserBitmap, getUserKindmap } from "../../store/newtimeTableStore"
...
const Profile = () => {
  ...
  const nextFreeTime = useMemo(() => {
    ...
    const formattedMinutes = minutes && !isNaN(parseInt(minutes, 10)) ? minutes : "00"

    return `${hour12}:${formattedMinutes} ${ampm}`
  }, [nextFreeTimeRaw])

  // Replace the direct function call with the hook
  const { data: timetableData, isLoading: timetableLoading, error: timetableError } = useUserTimetable()
    ...
    return (
      <div className="w-screen h-full flex flex-col overflow-y-auto scrollbar-hide">
        <div className="flex h-fit w-full gap-2 uppercase">
          <div className="ml-4 w-1/2 flex flex-col gap-2">
            ...
            <div className="p-4 bg-white text-black flex flex-col w-full flex-2 rounded-xl justify-center">
              ...
              <ul className="list-disc pl-5">{userData.data?.h?.map((hobby: string, index: number) => <li key={index}>{hobby}</li>) || <li>No hobbies listed</li>}</ul>
            </div>
          </div>
        ...
        <div className="mx-4 p-8 bg-white text-black rounded-xl text-center">
          ...
          <div className="text-sm text-gray-600 mt-2">Upload your timetable to see your schedule</div>
        </div>
        <div
      ...
      <div className="flex w-full gap-2 uppercase">
        <div className="ml-4 w-1/2 flex flex-col gap-2">
          ...
          <div className="p-4 bg-white text-black flex flex-col w-full flex-2 rounded-xl gap-4">
            ...
            <ul className="list-disc pl-5">{userData.data?.h?.map((hobby, index) => <li key={index}>{hobby}</li>) || <li>No hobbies listed</li>}</ul>
          </div>
        </div>
        <div className="mr-4 w-1/2 flex flex-col gap-2">
          <div className="p-4 bg-white text-black flex flex-col w-full flex-2 rounded-xl justify-center">
          ...
          <div className="p-4 bg-primary text-black flex flex-col w-full flex-1 rounded-xl justify-center">
            <div className="text-xl">NEXT FREE</div>
            <div className="text-3xl">{bitmapLoading || kindmapLoading || nextFreeLoading ? "Loading..." : nextFreeTime || "FREE DAY"}</div>
          </div>
        </div>
      </div>
