...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let base64_json = base64::encode(&json);
    
    // Save base64 encoded JSON to file
    let mut b64file = File::create("b64timetable_minimal.json")?;
    b64file.write_all(base64_json.as_bytes())?;
    println!("Base64 encoded timetable saved to b64timetable_minimal.json");
