...
const THEORY_PERIODS: [(u8, &str, &str); 12] = [
    (1, "08:00", "08:50"), (2, "08:55", "09:45"), (3, "09:50", "10:40"),
    (4, "10:45", "11:35"), (5, "11:40", "12:30"), (6, "12:35", "13:25"),
    (7, "14:00", "14:50"), (8, "14:55", "15:45"), (9, "15:50", "16:40"),
    (10, "16:45", "17:35"), (11, "17:40", "18:30"), (12, "18:35", "19:25"),
];

const LAB_PERIODS: [(u8, &str, &str); 12] = [
    (1, "08:00", "08:50"), (2, "08:50", "09:40"), (3, "09:50", "10:40"),
    (4, "10:40", "11:30"), (5, "11:40", "12:30"), (6, "12:30", "13:20"),
    (7, "14:00", "14:50"), (8, "14:50", "15:40"), (9, "15:50", "16:40"),
...
fn get_slot_index(time: NaiveTime) -> Option<usize> {
    ...
    if total_minutes >= 840 {  // After 14:00
...
fn build_bitmap(schedule: &[ScheduleEntry]) -> (Vec<u8>, HashMap<usize, String>) {
    ...
    for entry in schedule {
        let periods = if entry.s == "t" { &THEORY_PERIODS } else { &LAB_PERIODS };
...
#[command]
pub fn is_student_free(students: Vec<Student>, student_id: String, time: String) -> Result<(bool, Option<String>), String> {
...
#[command]
pub fn find_free_students(students: Vec<Student>, time: String) -> Result<Vec<(String, Option<String>)>, String> {
    let time = NaiveTime::parse_from_str(&time, "%H:%M")
        .map_err(|_| "Invalid time format")?;
...
#[command]
pub fn find_mutual_free(students: Vec<Student>, student_id1: String, student_id2: String) -> Result<Vec<FreeTimeSlot>, String> {
    let student1 = students.iter().find(|s| s.u == student_id1)
        .ok_or("Student 1 not found")?;
    let student2 = students.iter().find(|s| s.u == student_id2)
        .ok_or("Student 2 not found")?;
    
    let (bitmap1, _) = build_bitmap(&student1.o);
    let (bitmap2, _) = build_bitmap(&student2.o);
    
    let mutual: Vec<u8> = bitmap1.iter().zip(&bitmap2)
        .map(|(a, b)| 1 - (a | b))
        .collect();
...
#[command]
pub fn earliest_common_free_time(students: Vec<Student>, student_ids: Vec<String>) -> Result<Option<String>, String> {
    let mut bitmaps = Vec::new();
    
    for id in &student_ids {
        let student = students.iter().find(|s| s.u == *id)
    ...
    for i in 0..130 {
        let all_free = bitmaps.iter().all(|bitmap| i < bitmap.len() && bitmap[i] == 0);
        if all_free {
            let base = NaiveTime::from_hms_opt(8, 0, 0).unwrap();
...
#[command]
pub fn students_free_for(students: Vec<Student>, duration_mins: u32, time: String) -> Result<Vec<String>, String> {
    let time = NaiveTime::parse_from_str(&time, "%H:%M")
        .map_err(|_| "Invalid time format")?;
    ...
    for student in &students {
        ...
        if start_idx + slots_needed <= bitmap.len() {
            let all_free = (start_idx..start_idx + slots_needed)
                .all(|i| bitmap[i] == 0);
...
#[command]
pub fn overbooked_students(students: Vec<Student>, threshold: u32) -> Result<Vec<(String, u32)>, String> {
...
#[command]
pub fn get_status_bitmap(user: Student, current_day: u8, current_time: String) -> Result<UserStatus, String> {
    let current_time = NaiveTime::parse_from_str(&current_time, "%H:%M")
        .map_err(|_| "Invalid time format")?;
