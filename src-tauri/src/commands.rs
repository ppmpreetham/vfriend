use crate::scheduling_conflict::{
    CompactTimetable, Friend, initialize_time_slots,
    initialize_lunch_periods, find_common_free_times, is_user_free_at, get_time_slot_info
};
use serde::Serialize;
...
#[tauri::command]
pub fn check_conflicts_debug(user1_schedule_json: &str, user2_schedule_json: &str) -> DebugConflictResult {
    ...
    debug_info.push_str(&format!("Parsed User1: {} classes\n", user1_schedule.o.len()));
    debug_info.push_str(&format!("Parsed User2: {} classes\n", user2_schedule.o.len()));
    
    // Debug the specific test case
    debug_info.push_str("\nUser1 classes:\n");
    for slot in &user1_schedule.o {
        if let Some((start, end)) = get_time_slot_info(slot.d, slot.p, &slot.s, &time_slots, &lunch_periods) {
    ...
    for day in 1..=7 {
        ...
        let user1_day_slots: Vec<_> = user1_schedule.o.iter().filter(|slot| slot.d == day).collect();
        let user2_day_slots: Vec<_> = user2_schedule.o.iter().filter(|slot| slot.d == day).collect();
        
        debug_info.push_str(&format!("User1 has {} slots on day {}\n", user1_day_slots.len(), day));
        debug_info.push_str(&format!("User2 has {} slots on day {}\n", user2_day_slots.len(), day));
        
        // Manual conflict checking with debug
        for slot1 in &user1_day_slots {
            if slot1.p == 0 { continue; }
            
            if let Some((slot1_start, slot1_end)) = get_time_slot_info(slot1.d, slot1.p, &slot1.s, &time_slots, &lunch_periods) {
                debug_info.push_str(&format!("  Checking User1 slot: {} - {} ({})\n", slot1_start, slot1_end, slot1.f));
                
                for slot2 in &user2_day_slots {
                    if slot2.p == 0 { continue; }
                    
                    if let Some((slot2_start, slot2_end)) = get_time_slot_info(slot2.d, slot2.p, &slot2.s, &time_slots, &lunch_periods) {
                        debug_info.push_str(&format!("    Against User2 slot: {} - {} ({})\n", slot2_start, slot2_end, slot2.f));
                        ...
                        debug_info.push_str(&format!("    Times in minutes: {}({}) - {}({}) vs {}({}) - {}({})\n", 
                            slot1_start, slot1_start_mins, slot1_end, slot1_end_mins,
                            slot2_start, slot2_start_mins, slot2_end, slot2_end_mins
                        ));
                        
                        let overlaps = slot1_start_mins < slot2_end_mins && slot1_end_mins > slot2_start_mins;
                        debug_info.push_str(&format!("    Overlap check: {} < {} && {} > {} = {}\n", 
                            slot1_start_mins, slot2_end_mins, slot1_end_mins, slot2_start_mins, overlaps
                        ));
...
#[tauri::command]
pub fn check_conflicts(user1_schedule_json: &str, user2_schedule_json: &str) -> Vec<ConflictResult> {
...
pub fn find_free_times(schedules_json: &str) -> Vec<FreeTimeResult> {
    ...
    let schedules: Vec<CompactTimetable> = serde_json::from_str(schedules_json)
        .unwrap_or_else(|_| Vec::new());
        
    let friends: Vec<Friend> = schedules.into_iter()
        .map(|schedule| Friend {
    ...
    let result: Vec<FreeTimeResult> = free_times.into_iter()
...
pub fn is_free_at(schedule_json: &str, day: u8, time: &str) -> bool {
    ...
    let schedule: CompactTimetable = serde_json::from_str(schedule_json)
